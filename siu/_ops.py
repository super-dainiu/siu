...
from colossalai.auto_parallel.passes.runtime_apply_pass import runtime_apply, runtime_apply_for_iterable_object, runtime_comm_spec_apply
...
def runtime_apply_for_iterable_object_impl(tensor_list, origin_dict, input_dict, node_index, user_node_index):
    ...
    return [MetaTensor(torch.empty(out_shape, device='meta', dtype=tensor.dtype), device=tensor.device) for out_shape, tensor in zip(out_shape_list, tensor_list)]
