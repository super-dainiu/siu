...
def _end_of_ckpt(node: Node, ckpt_level: int) -> bool:
    ...
    Args:
        ...
        ckpt_level (int): the index of checkpoint level for
        nested checkpoint
    ...
    if len(n_info.to_recompute) > ckpt_level:
        return n_info.to_recompute[ckpt_level] is not None
    return True
...
def emit_ckpt_func(body,
    ...
    if isinstance(MetaInfo(node_list[0]).to_recompute, int):
    ...
    # use nested ckpt function codegen
        ...
        label = "_".join([str(idx) for idx in MetaInfo(node_list[0]).to_recompute[:level + 1]])
        ckpt_fn_def = _gen_ckpt_fn_def(label, inputs)
        ckpt_func.append(f'{ckpt_fn_def}\n')

        # if there is more level to fetch
        if level + 1 < len(MetaInfo(node_list[0]).to_recompute):
...
def _find_nested_ckpt_regions(nodes: Node, ckpt_level: int = 0):
    """
    ...
    for idx, node in enumerate(nodes):
        if 'activation_checkpoint' in node.meta:
            if isinstance(node.meta['activation_checkpoint'], int):
                ...
                act_ckpt_label = node.meta['activation_checkpoint'][ckpt_level]
